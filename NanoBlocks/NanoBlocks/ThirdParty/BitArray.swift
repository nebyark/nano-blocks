...
public struct BitArray {
    ...
    // Constructs a bit array from a UInt32 value.
    public init(value: UInt32, count: Int) {
        for i in (0..<32) {
            append((1 << i & value) != 0 ? true : false)
        }
    }
    ...
    public init(repeating repeatedValue: Bool, count: Int) {
        ...
        let intValue: Int = repeatedValue ? ~0 : 0
        self.bits = [Int](repeating: intValue, count: numberOfInts)
        self.count = count
    ...
    public var byteArray: [UInt8] {
        var result: [UInt8] = []
        var inc: Int = 0
        while inc < self.count {
            let start = self.index(inc, offsetBy: 0)
            let end = self.index(inc + 8, offsetBy: 0)
            let substring = self[start..<end]
            let bitSequence = substring.map({ UInt8($0 ? 1 : 0) })
            result.append(UInt8.from(sequence: bitSequence))
            inc += 8
        }
